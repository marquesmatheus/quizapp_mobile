...
import com.profdata.quiz.Interface.IAnswerSelect
...
class QuestionFragment : Fragment(), IAnswerSelect{
    override fun selectedAnswer(): CurrentQuestion {
        ...
        if(Common.answerSheetList[questionIndex].type == Common.ANSWER_TYPE.NO_ANSWER)
        {
            ...
            if(Common.selected_values.size > 1) // se multipla escolha
            {
                val arrayAnswer = Common.selected_values.toTypedArray()

                //Aqui nós vamos pegar o primeiro caracter de cada resposta
                //Ex: arr[0] = A. Paris
                // arr[1] = B. New York
                // CorrectAnswer = A,B
            ...
            }else if(Common.selected_values.size==1) //Se tiver somente uma resposta
            {
                val arrayAnswer = Common.selected_values.toTypedArray()
            ...
            Common.selected_values.clear() // limpar a seleção do array
